...
import os
from SSCFormer.evaluation.model_selection.summarize_results_in_one_json import summarize
...
if __name__ == "__main__":
    ...
    base_dir = "G:/CY/CodesOfCy/new/SSCFormer-main/DATASET/SSCFormer_trained_models/SSCFormer"
    SSCFormers = ['SSCFormerV2', 'SSCFormerV2_zspacing']
    ...
    with open("summary.csv", 'w') as f:
        ...
        for i in task_ids:
            for SSCFormer in SSCFormers:
                try:
                    summary_folder = join(base_dir, SSCFormer, "summary_fold0")
                    if isdir(summary_folder):
                        ...
                        for s in summary_files:
                            ...
                            expected_output_folder = join(base_dir, SSCFormer, tmp[1], tmp[0], tmp[2].split(".")[0])
                            name = tmp[0] + "__" + SSCFormer + "__" + tmp[1] + "__" + tmp[2].split(".")[0]
                            global_dice_json = join(base_dir, SSCFormer, tmp[1], tmp[0], tmp[2].split(".")[0], "fold_0", "validation_tiledTrue_doMirror_True", "global_dice.json")

                            if not isdir(expected_output_folder) or len(tmp) > 3:
                                if len(tmp) == 2:
                                    continue
                                expected_output_folder = join(base_dir, SSCFormer, tmp[1], tmp[0], tmp[2] + "__" + tmp[3].split(".")[0])
                                name = tmp[0] + "__" + SSCFormer + "__" + tmp[1] + "__" + tmp[2] + "__" + tmp[3].split(".")[0]
                                global_dice_json = join(base_dir, SSCFormer, tmp[1], tmp[0], tmp[2] + "__" + tmp[3].split(".")[0], "fold_0", "validation_tiledTrue_doMirror_True", "global_dice.json")
