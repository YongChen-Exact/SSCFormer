...
import importlib
import pkgutil
from SSCFormer.training.network_training.SSCFormerTrainer import SSCFormerTrainer
from SSCFormer.training.network_training.SSCFormerTrainerV3 import SSCFormerTrainerV3

from SSCFormer.training.network_training.SSCFormerTrainer_synapse import SSCFormerTrainer_synapse
from SSCFormer.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    """
    This is a utility function to load any SSCFormer trainer from a pkl. It will recursively search
    SSCFormer.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    is specified, it will furthermore load the checkpoint file in train/test mode (as specified by train).
    The pkl file required here is the one that will be saved automatically when calling SSCFormerTrainer.save_checkpoint.
    :param pkl_file:
    :param checkpoint:
    :param train:
    ...
    if 'nnUNet' in name:
        name=name.replace('nnUNet','SSCFormer')
    if len(init)>10:
        init=list(init)
        del init[2]
        del init[-2]
    search_in = join(SSCFormer.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="SSCFormer.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in SSCFormer.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within SSCFormer.trainig.network_training (search is recursive)."
                           "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s " % (checkpoint, name))
    assert issubclass(tr, SSCFormerTrainer) or issubclass(tr, SSCFormerTrainer_synapse) or issubclass(tr,SSCFormerTrainerV3), "The network trainer was found but is not a subclass of SSCFormerTrainer.Please make it so!"

    # this is now deprecated
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of SSCFormer. Attempting to load it "
...
if __name__ == "__main__":
    pkl = "/home/fabian/PhD/results/SSCFormerV2/SSCFormerV2_3D_fullres/Task004_Hippocampus/fold0/model_final_checkpoint.pkl"
    checkpoint = pkl[:-4]
    train = False
