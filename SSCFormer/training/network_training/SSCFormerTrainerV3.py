...
from SSCFormer.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from SSCFormer.training.loss_functions.dice_loss import DC_and_CE_loss
from SSCFormer.training.network_training.network_trainer_synapse import NetworkTrainer
...
# 和普通的SSCFormerTrainer的区别是修改类别问题：Synapse本身是14类，但是论文中只需要8类，所以在最后的类别设置那儿修改了num_class
class SSCFormerTrainerV3(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        IMPORTANT: If you inherit from SSCFormerTrainer and the init args change then you need to redefine self.init_args
        in your init accordingly. Otherwise checkpoints won't load properly!
        """
        super(SSCFormerTrainerV3, self).__init__(deterministic, fp16)
    ...
    def run_training(self):
        ...
        super(SSCFormerTrainerV3, self).run_training()
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class([join(SSCFormer.__path__[0], "preprocessing")],
                                                         preprocessor_name,
                                                         current_module="SSCFormer.preprocessing")
        assert preprocessor_class is not None, "Could not find preprocessor %s in SSCFormer.preprocessing" % \
                                               preprocessor_name
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(SSCFormerTrainerV3, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
