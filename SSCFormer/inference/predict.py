...
from SSCFormer.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
from SSCFormer.postprocessing.connected_components import load_remove_save, load_postprocessing
from SSCFormer.training.model_restore import load_model_and_checkpoint_files
...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, SSCFormerTrainer) or isinstance(trainer, SSCFormerTrainerV3)
    q = Queue(1)
    processes = []
